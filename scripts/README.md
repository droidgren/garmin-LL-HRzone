...
`LastLapHRZone/bin/device-tests/<timestamp>/`

## Quick start

From repo root (`d:\garmin-LL-HRzone`):
...
These tasks read SDK/key from `LastLapHRZone/.vscode/ciq.local.json`.
...
```powershell
.\scripts\Test-ConnectIQDeviceMatrix.ps1 -DeveloperKeyPath .\developer_key.der -Mode Core -OutputPath .\LastLapHRZone\bin\device-tests\latest
```
...
- `-ProjectPath`: path to app folder (default: `..\LastLapHRZone` relative to script)
